...
from django.db.models import OuterRef, Subquery, Prefetch, Q, Count, Sum
...
class UserLessonsViewSet(ListModelMixin, GenericViewSet):
    ...
    def get_queryset(self):
        ...
        return Lesson.objects.prefetch_related(
            Prefetch('related_views', LessonViewing.objects.filter(user = user))
        ).filter(Q(products__in=product_ids)).annotate(
            product = Coalesce(
                Subquery(
...
class UserProductLessonsViewSet(ListModelMixin, GenericViewSet):
    ...
    def get_queryset(self):
        ...
        return Lesson.objects.prefetch_related(
            Prefetch('related_views', LessonViewing.objects.filter(user=user))
            ).filter(Q(products = product)).annotate(
                duration_viewed=Coalesce(
                    Subquery(
                        LessonViewing.objects.filter(
                            lesson=OuterRef('pk'),
